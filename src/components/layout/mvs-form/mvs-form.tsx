...
@Component({
	tag: 'mvs-form',
	styleUrl: 'mvs-form.scss',
	shadow: true,
})
export class MvsForm {
	@State() email: any = {
		name: '',
		email: '',
		phone: '',
	...
	setEmailArray(name, event) {
		if (event.target.checked)
			this.email = {
				...this.email,
				[name]: [...this.email[name], event.target.value],
			}
		else {
			...
			<form method='POST'>
				<mvs-grid class='container'>
					...
					<div class='col'>
						...
						<input type='phone' onChange={e => this.setEmail('phone', e)} />
					</div>
					<div class='col'>
						<label>Assunto</label>
						<input
					...
					<div class='col'>
						...
						<select
							name=''
							id=''
							onChange={e => this.setEmail('typeVehicle', e)}
						>
					...
					<div class='col'>
						...
						<input
							type='checkbox'
							value='juiz'
							onClick={e => this.setEmailArray('profession', e)}
						/>
						Juiz <br />
						<input
							type='checkbox'
